...
module YesnoGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YesnoGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = YesnoGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YesnoGeneratorResponse.new(fetched)
    else
