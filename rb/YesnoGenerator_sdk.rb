...
require_relative 'YesnoGenerator_types'
...
class YesnoGeneratorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = YesnoGeneratorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YesnoGeneratorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, YesnoGeneratorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    YesnoGeneratorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = YesnoGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = YesnoGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = YesnoGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => YesnoGeneratorError.new(
        "#{op}_allow",
        "YesnoGeneratorSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue YesnoGeneratorError => err
    ...
    if fetched.is_a?(Hash)
      status = YesnoGeneratorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YesnoGeneratorError.new(
        "graphql_error", "YesnoGeneratorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = YesnoGeneratorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
