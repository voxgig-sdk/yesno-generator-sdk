...
class ApiEntity
  ...
  def data_set(args)
    if args
      @_data = YesnoGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = YesnoGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Api, Hash] the loaded Api; raises YesnoGeneratorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = YesnoGeneratorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
