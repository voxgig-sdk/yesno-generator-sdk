# YesnoGenerator SDK test runner
...
class YesnoGeneratorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YesnoGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YesnoGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YesnoGeneratorTestRunner.getenv("YESNO_GENERATOR_TEST_LIVE")
        override = YesnoGeneratorTestRunner.getenv("YESNO_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YesnoGeneratorTestRunner.getenv(key)
        ...
        explain = YesnoGeneratorTestRunner.getenv("YESNO_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if YesnoGeneratorTestRunner._test_control is not None:
            return YesnoGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YesnoGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YesnoGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YesnoGeneratorTestRunner.load_test_control()
...
def load_env_local():
    YesnoGeneratorTestRunner.load_env_local()


def env_override(m):
    return YesnoGeneratorTestRunner.env_override(m)


def entity_data(v):
    return YesnoGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YesnoGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YesnoGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YesnoGeneratorTestRunner.load_test_control()


def live_client_options():
    return YesnoGeneratorTestRunner.live_client_options()


def live_delay_ms():
    return YesnoGeneratorTestRunner.live_delay_ms()
