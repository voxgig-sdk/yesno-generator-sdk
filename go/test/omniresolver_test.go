...
import (
	...
	omni "github.com/voxgig-sdk/yesno-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YesnoGeneratorError); is {
				out := map[string]any{
					"name":    "YesnoGeneratorError",
...
func omniCtx(arg any, client *sdk.YesnoGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YesnoGeneratorSDK); is {
			client = live
