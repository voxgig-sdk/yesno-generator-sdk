...
//
// NewYesnoGeneratorSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.YesnoGeneratorSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewYesnoGeneratorSDK(opts)
}
...
func fcCandidates(client *sdk.YesnoGeneratorSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.YesnoGeneratorSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.YesnoGeneratorSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.YesnoGeneratorError
