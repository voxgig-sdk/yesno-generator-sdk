...
//     (\`new YesnoGeneratorSDK(...)\` / \`YesnoGeneratorSDK.test(...)\` become
//     \`YesnoGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'YesnoGeneratorSDK'
...
// rewrite real client construction (\`new YesnoGeneratorSDK(...)\` and any
// \`YesnoGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YesnoGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YesnoGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
