...
class YesnoGeneratorMakeError
{
    public static function call(?YesnoGeneratorContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new YesnoGeneratorContext([], null);
        }
        $op = $ctx->op ?? new YesnoGeneratorOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new YesnoGeneratorResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof YesnoGeneratorError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new YesnoGeneratorError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof YesnoGeneratorError) {
