...
//    only - a typed YesnoGeneratorContext there would make every ctx
...
final class YesnoGeneratorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new YesnoGeneratorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new YesnoGeneratorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new YesnoGeneratorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new YesnoGeneratorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new YesnoGeneratorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?YesnoGeneratorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new YesnoGeneratorError(is_string($code) ? $code : '', $msg);
    }
