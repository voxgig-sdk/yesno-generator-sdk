...
// YesnoGenerator SDK feature test
//
...
class FtCtrl extends YesnoGeneratorControl
{
...
class FtClient
{
    ...
    private YesnoGeneratorUtility $utility;

    public function __construct(YesnoGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): YesnoGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YesnoGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YesnoGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YesnoGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YesnoGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(YesnoGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YesnoGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YesnoGeneratorResponse($fetched);
        }
    ...
    private function fail(YesnoGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YesnoGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
