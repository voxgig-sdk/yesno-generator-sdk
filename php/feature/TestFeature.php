...
// YesnoGenerator SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class YesnoGeneratorTestFeature extends YesnoGeneratorBaseFeature
{
    ...
    public function init(YesnoGeneratorContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (YesnoGeneratorContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = YesnoGeneratorParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (YesnoGeneratorContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(YesnoGeneratorContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = YesnoGeneratorParam::call($ctx, $k);
