...
// YesnoGenerator SDK retry feature
...
class YesnoGeneratorRetryFeature extends YesnoGeneratorBaseFeature
{
    ...
    public function init(YesnoGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YesnoGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YesnoGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YesnoGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
